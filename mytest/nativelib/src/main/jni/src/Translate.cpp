...
int tmain(const char* inFilePath,const char* outFilePath)
{
    int ret;
    AVPacket packet;
    AVFrame *frame= NULL;
    ...
    if ((ret = open_input_file(inFilePath)) < 0)
        goto end;
    if ((ret = open_output_file(outFilePath)) < 0)
        goto end;
...
int Translate::translate(JNIEnv *env, jobject thiz, jstring filePath, jstring outPath) {
    const char* inFilePath = env->GetStringUTFChars(filePath,0);
    const char* outFilePath = env->GetStringUTFChars(outPath,0);
    return tmain(inFilePath,outFilePath);
}
